...
class Disc(Problem):
    ...
    def successor(self, state):
        ...
        for index,element in enumerate(state):
            if(element==0):
                continue
            if(index+1<self.l and state[index+1]==0):
                swap=list(state)
                swap[index],swap[index+1]=swap[index+1],swap[index]
                successors[f"D1: Disk {element}"]=tuple(swap)
            if(index+2<self.l and state[index+1]!=0 and state[index+2]==0):
                swap=list(state)
                swap[index],swap[index+2]=swap[index+2],swap[index]
                successors[f"D2: Disk {element}"]=tuple(swap)
            if(index-1>=0 and state[index-1]==0):
                swap=list(state)
                swap[index],swap[index-1]=swap[index-1],swap[index]
                successors[f"L1: Disk {element}"]=tuple(swap)
            if(index-2>=0 and state[index-1]!=0 and state[index-2]==0):
                swap=list(state)
                swap[index],swap[index-2]=swap[index-2],swap[index]
                successors[f"L2: Disk {element}"]=tuple(swap)
    ...
    def h(self,node):
        out_of_place=0
        list_goal=list(self.goal)
        lista=list(node.state)
        for x in lista:
            if x==0:
                continue
            if lista.index(x)!=list_goal.index(x):
                out_of_place+=1
        return out_of_place
...
if __name__ == "__main__":
    ...
    diskovi_reversed=list()
    for x in range(1,n+1):
        diskovi_reversed.append(x)
    for x in range(n,L):
        diskovi_reversed.append(0)

    diskovi_reversed.reverse()
    state=(tuple(diskovi))
    resenie=Disc(state,tuple(diskovi_reversed),L)
