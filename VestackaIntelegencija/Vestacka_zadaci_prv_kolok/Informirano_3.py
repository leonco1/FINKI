...
class Pacman(Problem):
    def __init__(self, initial,):
        ...
        self.precki=[(0,6),(1,2),(1,3),(1,4),(0,8),(0,9),(1,9),(2,9),(3,9),(6,9),(3,6),(4,6),(4,5),(4,7),(5,6),(6,0),(6,1),(6,2),
                     (5,1),(4,1),(8,1),(8,4),(9,4),(8,7),(8,8),(9,8),(9,7)
            ]

    def eat(self,state,tocki):
        pacman=state
        tocki=list(tocki)
        for x in tocki:
            if x==pacman:
    ...
    def check(self,state):
        pac_man = state
        if pac_man[0] >= 0 and pac_man[0] < 10 and pac_man[1] >= 0 and pac_man[1] < 10 and (pac_man[0],pac_man[1])not in self.precki:
            return True
        return False
    ...
    def successor(self, state):
        ...
        direction=pacman[2]
        if(direction=="istok"):
            new_poz=(pacman_x+1,pacman_y,direction)
            if self.check(new_poz):
                new_tocki=tocki
                if(new_poz[0],new_poz[1]) in new_tocki:
                    new_tocki=self.eat((new_poz[0],new_poz[1]),new_tocki)
                successors["ProdolzhiPravo"]=(new_poz,new_tocki)
                    #Nazad
            new_poz=(pacman_x-1,pacman_y,"zapad")
            if self.check(new_poz):
                new_tocki=tocki
                if(new_poz[0],new_poz[1]) in new_tocki:
                    new_tocki=self.eat((new_poz[0],new_poz[1]),new_tocki)
                successors["ProdolzhiNazad"]=(new_poz,new_tocki)
            #SvrtiLevo
            new_poz = (pacman_x, pacman_y+1, "sever")
            if self.check(new_poz):
                new_tocki = tocki
                if (new_poz[0], new_poz[1]) in new_tocki:
                    new_tocki = self.eat((new_poz[0], new_poz[1]), new_tocki)
                successors["SvrtiLevo"] = (new_poz, new_tocki)
                    #SvrtiDesno
            new_poz = (pacman_x, pacman_y -1, "jug")
            if self.check(new_poz):
                new_tocki = tocki
                if (new_poz[0], new_poz[1]) in new_tocki:
                    new_tocki = self.eat((new_poz[0], new_poz[1]), new_tocki)
                successors["SvrtiDesno"] = (new_poz, new_tocki)
        #Zapad
            ...
            if self.check(new_poz):
                ...
                successors["ProdolzhiPravo"] = (new_poz, new_tocki)
                    # Nazad
            new_poz = (pacman_x +1, pacman_y, "istok")
            if self.check(new_poz):
                new_tocki = tocki
                if (new_poz[0], new_poz[1]) in new_tocki:
                    new_tocki = self.eat((new_poz[0], new_poz[1]), new_tocki)
        ...
        #Sever
            ...
            new_poz = (pacman_x , pacman_y+1, direction)
            if self.check(new_poz):
                new_tocki = tocki
                if (new_poz[0], new_poz[1]) in new_tocki:
                    new_tocki = self.eat((new_poz[0], new_poz[1]), new_tocki)
    ...
    def h(self,node):
        if(len(node.state[1])==0):
            ...
            x=node.state[0][0]
            y=node.state[0][1]
            return max(abs(x-i)+abs(y-j) for i,j in node.state[1])
...
if __name__ == "__main__":
    ...
    for i in range(n):
        ...
        y=int(tocka.split(",")[1])
        tocki.append((x,y))
    initial_state=((pac_x,pac_y,nasoka),tuple(tocki))
    resenie=Pacman(initial_state)
    print(astar_search(resenie).solution())
