...
def trainee_factory(set):
    ...
    return x,y
def calculate_accuracy(X,Y,classficator):
    acc=0
    for x,y in zip(X,Y):
        predict=classficator.predict([x])[0]
        if(predict==y):
            acc+=1
    return acc/len(Y)
if __name__ == '__main__':
    ...
    classifier=MLPClassifier(int(input()),activation="relu",learning_rate_init=float(input()),random_state=0,max_iter=20)
    classifier.fit(train_x,train_y)
    train_accuracy=calculate_accuracy(train_x,train_y,classifier)
    test_accuracy=calculate_accuracy(test_x,test_y,classifier)
    col_index=int(input())
    primerok=list(map(float,input().split(" ")))
    if(train_accuracy>=1.15*test_accuracy):
        print("Se sluchuva overfitting")
        print("1")
    else:
        print("Ne se sluchuva overfitting")
        print(classifier.predict([primerok])[0])
