...
col_index=int(input())
dataset_new=[row[0:col_index]+row[col_index+1:]for row in dataset]
train_set=dataset_new[:int(0.85*len(dataset_new))]
test_set=dataset_new[int(0.85*len(dataset_new)):]
...
zapisi=list(map(float,input().split(" ")))
zapisi.pop(col_index)
print(classifier.predict([zapisi])[0])
print(classifier.predict_proba([zapisi])[0])
