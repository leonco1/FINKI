...
def divideclasses(col_index):
    class1=[el for el in dataset if el[-1]==col_index]
    return class1
...
if __name__ == '__main__':
    x1=float(input())
    x2=float(input())
    class1=divideclasses(0)
    class2=divideclasses(1)
    class3=divideclasses(2)
    train_bayes=class1[:int(x1*len(class1))]+class2[:int(x1*len(class2))]+class3[:int(x1*len(class3))]
    train_decision_tree=class1[int(x1*len(class1)):int(x2*len(class1))]+class2[int(x1*len(class2)):int(x2*len(class2))]+class3[int(x1*len(class3)):int(x2*len(class3))]
    train_forest=class1[:int(x2*len(class1))]+class2[:int(x2*len(class2))]+class3[:int(x2*len(class3))]
    test_set=class1[int(x2*len(class1)):]+class2[int(x2*len(class2)):]+class3[int(x2*len(class3)):]
    bayes_x,bayes_y=trainee_factory(train_bayes)
    decision_tree_x,decision_tree_y=trainee_factory(train_decision_tree)
    random_forest_x,random_forest_y=trainee_factory(train_forest)
    test_x,test_y=trainee_factory(test_set)
    bayes_classifer=GaussianNB()
    decision_tree_classifier=DecisionTreeClassifier()
    random_forest_classifier=RandomForestClassifier()
    bayes_classifer.fit(bayes_x,bayes_y)
    decision_tree_classifier.fit(decision_tree_x,decision_tree_y)
    random_forest_classifier.fit(random_forest_x,random_forest_y)
    accuracy=0
    for x,y in zip(test_x,test_y):
        predict_bayes=bayes_classifer.predict([x])[0]
        predict_decision_tree=decision_tree_classifier.predict([x])[0]
        predict_random_forest=random_forest_classifier.predict([x])[0]
        brojac=0
        if(predict_bayes==y):
            brojac+=1
        if(predict_random_forest==y):
            brojac+=1
        if(predict_decision_tree==y):
            brojac+=1
        if(brojac>=2):
            accuracy+=1
