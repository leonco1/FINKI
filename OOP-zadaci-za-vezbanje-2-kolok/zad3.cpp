...
public:
   ...
   virtual int getBodovi()
   {
       ...
       double average=(1.0*ocenka)/brojOcenki;
       int bodovi=average*100;
       return bodovi;
...
private:
    int brojcasovi;
public:
    Demonstrator(int indeks,int *ocenki, int brojOcenki,char *imeIPrezime,Kurs *kursevi,int brojKursevi,int brojcasovi=0):Student(indeks,ocenki,brojOcenki),Predavach(imeIPrezime,kursevi,brojKursevi)
    {
...
Student& vratiNajdobroRangiran(Student **studenti,int n)
{
    ...
    for(int i=0;i<n;i++)
    {
        if(studenti[i]->getBodovi()>maxPoeni)
        {
            najdobar=i;
            maxPoeni=studenti[i]->getBodovi();
        }
    }
